...
function printRecettes($rubrique, $debut, $fin){
    ...
    $infosRecette = getDetailsRecettes($dataQ['rubrique']);
    ...
    $worksheet->getCell('A6')->setValue('RAPPORT DES RECETTES : ' . $infosRecette['designation'] );
    
    $worksheet->getCell('L7')->setValue($debut);
    
    $worksheet->getCell('L8')->setValue($fin);
    
    $worksheet->getCell('D7')->setValue($infosRecette['intitule']);    
    
    $worksheet->getCell('D8')->setValue($infosRecette['montant'] . ' ' . $infosRecette['monnaie']);
    ...
    foreach ($etudiants as $key => $value) {  
        ...
        $soldeEtudiant = getSoldeOfEtudiant($value['id'], $dataQ['rubrique']);
        
        $worksheet->getCell('A' . $row)->setValue($i);     
    
        $worksheet->getCell($col[0] . $row)->setValue($value['nom'] . ' ' . $value['post_nom'] . ' ' . $value['prenom']);
    
        $worksheet->getCell($col[1] . $row)->setValue((isset($soldeEtudiant['TOTAL'])) ? $soldeEtudiant['TOTAL'] . ' ' . $infosRecette['monnaie'] : '0' . ' ' . $infosRecette['monnaie']);

        $worksheet->getCell($col[2] . $row)->setValue((isset($soldeEtudiant['RESTE'])) ? $soldeEtudiant['RESTE'] . ' ' . $infosRecette['monnaie']: '0' . ' ' . $infosRecette['monnaie']);
        
        $row++;

        if (isset($soldeEtudiant['TOTAL'])) {
            $total += $soldeEtudiant['TOTAL'];
            $paiements += 1;

            if ($soldeEtudiant['TOTAL'] < $infosRecette['montant']) {
                $dettes += 1;
            } else {
                $dettes += 0;
                $regulier += 1;
        ...
        if (isset($soldeEtudiant['RESTE'])) {
            $reste += $soldeEtudiant['RESTE'];
    ...
    $worksheet->getCell('L9')->setValue($i * $infosRecette['montant'] . ' ' . $infosRecette['monnaie']);

    $worksheet->getCell('L10')->setValue($total . ' ' . $infosRecette['monnaie']);

    $worksheet->getCell('L11')->setValue($reste. ' ' . $infosRecette['monnaie']);

    $worksheet->getCell('L12')->setValue($irregulier);

    $worksheet->getCell('L13')->setValue($paiements);

    $worksheet->getCell('D9')->setValue(round($regulier*100/$i,3));

    $worksheet->getCell('D10')->setValue(round($irregulier*100/$i,3));

    $worksheet->getCell('D11')->setValue($infosRecette['semestre']);

    $worksheet->getCell('D12')->setValue($i);
...
function perception($id){
    //Liste des étudiants
    $detail_rubrique = getStudentByRubrique($id);
    
    $infos_rubrique = getRubrique($id);

	//Lien vue - controleur
	require "vue/pages/perception.php";
